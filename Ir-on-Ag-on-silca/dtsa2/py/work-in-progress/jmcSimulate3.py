# -*- coding: utf-8 -*-
# DTSA-II/NISTMonte script - Nicholas W. M. Ritchie - 6-May-2010. JM test version 2018-07-25 to return both detected and emitted spectra
...
def base(det, e0, withPoisson, nTraj, dose, sf, bf, name, buildSample, buildParams, xtraParams):
    ...
    # jrm
    rawspec = det.getRawXRayData((dose * 1.0e-9) / (nTraj * epq.PhysicalConstants.ElectronCharge))
    ...
    # jrm
    props = rawspec.getProperties()
    props.setNumericProperty(epq.SpectrumProperties.LiveTime, dose)
    props.setNumericProperty(epq.SpectrumProperties.FaradayBegin, 1.0)
    props.setNumericProperty(epq.SpectrumProperties.BeamEnergy, e0)
    epq.SpectrumUtils.rename(rawspec, name)
    if withPoisson:
    ...
    return [dtsa2.wrap(spec), dtsa2.wrap(rawspec)]
...
def simulate(mat, det, e0=20.0, dose=defaultDose, withPoisson=True, nTraj=defaultNumTraj, sf=defaultCharFluor, bf=defaultBremFluor, xtraParams=defaultXtraParams):
    ...
    res[0].getProperties().setCompositionProperty(epq.SpectrumProperties.StandardComposition, mat)
    res[1].getProperties().setCompositionProperty(epq.SpectrumProperties.StandardComposition, mat)
    return res
...
def multiFilm(layers, det, e0=20.0, withPoisson=True, nTraj=defaultNumTraj, dose=defaultDose, sf=defaultCharFluor, bf=defaultBremFluor, xtraParams=defaultXtraParams):
    ...
    res = base(det, e0, withPoisson, nTraj, dose, sf, bf, tmp, buildFilm, {"Layers": layers }, xtraParams)
    return res
